...
def isolate_identity_db(tmp_path):
    """Isolate each test with its own temporary identity database."""
    reset_identity_manager()
    test_db = tmp_path / "identity.db"
    manager = LocalIdentityManager(db_path=test_db)
    yield manager
    reset_identity_manager()
...
class TestLocalIdentityCreation:
    ...
    def test_identity_manager_creation(self, isolate_identity_db):
        """Identity manager can be created."""
        manager = isolate_identity_db
        assert manager is not None

    def test_step_up_initially_inactive(self, isolate_identity_db):
        """Step-up should be inactive initially."""
        manager = isolate_identity_db
        assert manager.is_step_up_active() is False

    def test_identity_not_locked_initially(self, isolate_identity_db):
...
class TestStepUpSuccess:
    ...
    def test_start_step_up_creates_challenge(self, isolate_identity_db):
    ...
    def test_complete_step_up_with_valid_pin(self, isolate_identity_db, mocker):
        """Valid PIN completes step-up successfully."""
        mock_pm = MagicMock()
        mock_pm.is_pin_set.return_value = True
        mock_pm.verify_pin.return_value = True
        mocker.patch("ai.settings_service.PINManager", return_value=mock_pm)
    ...
    def test_step_up_expires_after_duration(self, isolate_identity_db, mocker):
...
class TestStepUpFailure:
    ...
    def test_complete_step_up_with_invalid_pin_fails(self, isolate_identity_db, mocker):
    ...
    def test_failed_attempts_counter(self, isolate_identity_db, mocker):
    ...
    def test_lockout_after_max_attempts(self, isolate_identity_db, mocker):
...
class TestTrustedDevice:
    ...
    def test_create_trusted_device(self, isolate_identity_db):
    ...
    def test_device_trusted_check(self, isolate_identity_db):
        """Can check if device is trusted."""
        manager = isolate_identity_db
        device_id = manager.create_trusted_device()
        assert manager.is_device_trusted(device_id) is True

    def test_revoke_trusted_device(self, isolate_identity_db):
    ...
    def test_get_trusted_devices(self, isolate_identity_db):
...
class TestStepUpRequiredOperations:
    ...
    def test_secret_reveal_requires_step_up(self, mocker):
        """Secret reveal requires step-up."""
        mock_pm = MagicMock()
        mock_pm.is_pin_set.return_value = True
        mock_pm.verify_pin.return_value = True
    ...
    def test_settings_mutation_requires_step_up(self, mocker):
        """Settings mutation requires step-up."""
        mock_pm = MagicMock()
        mock_pm.is_pin_set.return_value = True
        mock_pm.verify_pin.return_value = True
    ...
    def test_high_risk_tool_requires_step_up(self, mocker):
        """High-risk P127 tool requires step-up."""
        mock_pm = MagicMock()
        mock_pm.is_pin_set.return_value = True
        mock_pm.verify_pin.return_value = True
...
class TestNoSecretsExposure:
    ...
    def test_no_raw_pin_in_results(self, isolate_identity_db):
    ...
    def test_no_raw_secrets_in_identity_state(self, isolate_identity_db):
...
class TestRequireStepUp:
    ...
    def test_require_step_up_when_inactive(self, isolate_identity_db):
        """require_step_up returns challenge when step-up inactive."""
        from ai.identity import models

        models._identity_manager = isolate_identity_db
    ...
    def test_require_step_up_when_active(self, isolate_identity_db):
        """require_step_up returns allowed when step-up active."""
        from ai.identity import models

        models._identity_manager = isolate_identity_db

        with patch.object(isolate_identity_db, "is_step_up_active", return_value=True):
            with patch.object(isolate_identity_db, "get_step_up_expires_at"):
                result = require_step_up("settings.reveal_secret")
                assert result["allowed"] is True

    def test_require_step_up_when_locked(self, isolate_identity_db):
        """require_step_up returns locked error when identity locked."""
        from ai.identity import models

        models._identity_manager = isolate_identity_db

        with patch.object(isolate_identity_db, "is_locked", return_value=True):
            with patch.object(isolate_identity_db, "get_locked_until", return_value=None):
                result = require_step_up("settings.reveal_secret")
                assert result["allowed"] is False
