...
export class ExoticBridge {
    ...
    qaoa(problem, config) {
        if (!this._module)
            throw new Error('Exotic module not initialized');
     ...
     */
    quantumAnnealing(problem, config) {
        if (!this._module)
            throw new Error('Exotic module not initialized');
        const mergedConfig = { ...this.config, ...config };
     ...
     */
    groverSearch(oracle, searchSpace, config) {
        if (!this._module)
            throw new Error('Exotic module not initialized');
        const mergedConfig = { ...this.config, ...config };
     ...
     */
    tensorContract(tensors, contractionOrder) {
        if (!this._module)
            throw new Error('Exotic module not initialized');
    ...
    createMockModule() {
        return {
            ...
            tensorContract(tensors, contractionOrder) {
                ...
                if (tensors.length === 1)
                    return new Float32Array(tensors[0]);
            ...
            amplitudeEstimation(statePrep, groverIterations) {
 ...
 */
export function createExoticBridge(config) {
