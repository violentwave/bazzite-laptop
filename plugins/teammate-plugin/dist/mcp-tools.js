 ...
 */
function validateStringParam(value, name, maxLength = MAX_PARAM_LENGTH) {
 ...
 */
function validateArrayParam(value, name, maxItems = MAX_ARRAY_ITEMS) {
...
export const TEAMMATE_MCP_TOOLS = [
    ...
    {
        name: 'teammate_spawn',
        description: 'Spawn a new teammate in a team. Returns AgentInput for Claude Code Task tool.',
        inputSchema: {
            type: 'object',
            properties: {
                teamName: {
    ...
    // ==========================================================================
    {
        ...
        description: 'Enable BMSSP-powered optimization for team topology and task routing. ' +
    ...
    },
    {
        ...
        description: 'Find optimal message routing path between two teammates using BMSSP graph algorithms.',
        inputSchema: {
            type: 'object',
            properties: {
                teamName: { type: 'string', description: 'Team name' },
    ...
    },
    {
        ...
        description: 'Get topology statistics and optimization suggestions for a team.',
        inputSchema: {
            type: 'object',
            properties: {
                teamName: { type: 'string', description: 'Team name' },
    ...
    },
    {
        ...
        description: 'Route a task to the best-suited teammate using semantic matching ' +
    ...
    },
    {
        ...
        description: 'Route multiple tasks to teammates optimally, avoiding over-assignment.',
        inputSchema: {
            type: 'object',
            properties: {
                teamName: { type: 'string', description: 'Team name' },
...
export async function handleMCPTool(bridge, toolName, params) {
    try {
        ...
        switch (toolName) {
            ...
            case 'teammate_spawn_team': {
                ...
                const config = {
                    ...
                    maxTeammates: params.maxTeammates ?? 8,
                    planModeRequired: params.planModeRequired ?? false,
                    autoApproveJoin: params.autoApproveJoin ?? true,
                    delegationEnabled: params.delegationEnabled ?? true,
                };
            ...
            case 'teammate_spawn': {
                ...
                const allowedTools = params.allowedTools
                    ? validateArrayParam(params.allowedTools, 'allowedTools', 50)
                    : undefined;
                const spawnConfig = {
                    name,
                    role,
                    ...
                    allowedTools,
                    mode: params.mode,
                    delegateAuthority: params.delegateAuthority,
            ...
            case 'teammate_send_message': {
                const message = await bridge.sendMessage(params.teamName, params.fromId, params.toId, {
                    type: params.type,
            ...
            case 'teammate_broadcast': {
                const message = await bridge.broadcast(params.teamName, params.fromId, {
                    type: params.type,
                    payload: params.payload,
                    priority: params.priority,
                });
            ...
            case 'teammate_submit_plan': {
                const team = bridge.getTeamState(params.teamName);
                const requiredApprovals = params.requiredApprovals ??
                    Math.ceil((team?.teammates.length ?? 1) / 2);
                const plan = await bridge.submitPlan(params.teamName, {
            ...
            case 'teammate_approve_plan': {
                await bridge.approvePlan(params.teamName, params.planId, params.approverId);
                const team = bridge.getTeamState(params.teamName);
                const plan = team?.activePlans.find(p => p.id === params.planId);
                return { success: true, data: { plan } };
            }
            case 'teammate_launch_swarm': {
                const exitPlanInput = await bridge.launchSwarm(params.teamName, params.planId, params.teammateCount);
                return {
                    success: true,
                    data: {
                        exitPlanInput,
            ...
            case 'teammate_delegate': {
                const delegation = await bridge.delegateToTeammate(params.teamName, params.fromId, params.toId, params.permissions);
                return { success: true, data: { delegation } };
            }
            // Context & Memory
            case 'teammate_update_context': {
                const context = await bridge.updateTeamContext(params.teamName, {
            ...
            case 'teammate_save_memory': {
                await bridge.saveTeammateMemory(params.teamName, params.teammateId);
                return { success: true, data: { saved: true } };
            }
            case 'teammate_share_transcript': {
                await bridge.shareTranscript(params.teamName, params.fromId, params.toId, {
                    start: params.start,
                    end: params.end,
                });
            ...
            case 'teammate_push_remote': {
                const remoteSession = await bridge.pushTeamToRemote(params.teamName);
            ...
            case 'teammate_teleport': {
                ...
                const result = await bridge.teleportTeam(params.teamName, target);
                return { success: result.success, data: result };
            }
            // Status & Cleanup
            case 'teammate_get_status': {
                const teamState = bridge.getTeamState(params.teamName);
                if (!teamState) {
            ...
            case 'teammate_cleanup': {
                await bridge.cleanup(params.teamName);
            ...
            case 'teammate_route_task': {
                ...
                const requiredSkills = validateArrayParam(params.requiredSkills, 'requiredSkills', 20);
            ...
            case 'teammate_batch_route': {
                ...
                const teamName = validateStringParam(params.teamName, 'teamName', 64);
                const tasks = validateArrayParam(params.tasks, 'tasks', 50);
...
// ============================================================================
export function listTeammateTools() {
    return TEAMMATE_MCP_TOOLS;
}
export function hasTeammateTool(name) {
