...
export class TopologyOptimizer {
    ...
    graph = null;
    nodeMap = new Map();
    reverseNodeMap = new Map();
    edges = [];
    nodeCount = 0;
    initialized = false;
    useFallback = false;
    // Fallback adjacency list for when WASM is unavailable
    fallbackAdjList = new Map();
    constructor(topology = 'mesh') {
     ...
     */
    findOptimalRoute(fromId, toId, teammates) {
        ...
        if (stats.density < 0.3) {
            ...
            for (const [id1] of this.nodeMap) {
                for (const [id2] of this.nodeMap) {
                    if (id1 >= id2)
                        continue;
                    const hasEdge = this.edges.some(e => (e.from === id1 && e.to === id2) || (e.from === id2 && e.to === id1));
                    if (!hasEdge) {
                        suggestedEdges.push({
                            from: id1,
                            to: id2,
    ...
    buildHierarchicalTopology(teammates) {
...
// Factory Function
// ============================================================================
export async function createTopologyOptimizer(topology = 'mesh') {
