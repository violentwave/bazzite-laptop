...
export class HottEngine {
     ...
     */
    async checkPathEquivalence(path1, path2) {
     ...
     */
    async transportAlong(path, value) {
        // Verify value type matches path source
        if (value.type !== path.type) {
            throw new Error(`Type mismatch: value has type ${value.type} but path is in type ${path.type}`);
        }
        if (this.wasmModule) {
     ...
     */
    async verifyProof(proposition, proof) {
        // Check cache
     ...
     */
    verifyProofJS(proposition, proof) {
        // Parse proposition
    ...
    applyTransport(path, value) {
        ...
        const transformedValue = this.substituteInValue(value.value, path.source, path.target);
        return {
     ...
     */
    substituteInValue(value, from, to) {
...
//# sourceMappingURL=HottEngine.js.map