 ...
 */
import { CohomologyEngine, SpectralEngine, CausalEngine, QuantumEngine, CategoryEngine, HottEngine } from './engines/index.js';
/**
...
const globalWindow = typeof globalThis !== 'undefined' ? globalThis.window : undefined;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const globalSelf = typeof globalThis !== 'undefined' ? globalThis.self : undefined;
...
export class WasmBridge {
    ...
    async loadWasmNode(wasmPath) {
        try {
            ...
            if (!path.default.isAbsolute(wasmPath)) {
                // Try node_modules
                const nodeModulesPath = path.default.join(process.cwd(), 'node_modules', wasmPath);
     ...
     */
    async loadWasmBrowser(wasmPath) {
        try {
    ...
    ensureInitialized() {
        if (!this.initialized) {
            throw new Error('WasmBridge not initialized. Call initialize() first or await the initialization promise.');
        }
    }
}
/**
 ...
 */
export function createWasmBridge(config) {
    return new WasmBridge(config);
}
/**
 * Create and initialize a WASM bridge
 */
export async function initializeWasmBridge(config) {
