 ...
 * @version 0.1.3
 */
import type { CoherenceCheckResult, SpectralAnalysisResult, CausalInferenceResult, CausalGraph, TopologyResult, MorphismResult, HottProofResult, ConsensusResult, AgentState, MemoryEntry, MemoryCoherenceValidation, CoherenceThresholds } from './types.js';
/**
...
export interface IPrimeRadiantBridge {
     ...
     */
    inferCausal(treatment: string, outcome: string, graph: CausalGraph): Promise<CausalInferenceResult>;
    /**
...
export interface IQuantumEngine {
     ...
     */
    wassersteinDistance?(diagram1: Array<[number, number]>, diagram2: Array<[number, number]>): number;
}
/**
...
export interface IStabilityAnalyzer {
     ...
     */
    detectTrends?(historicalMetrics: SpectralAnalysisResult[]): Promise<{
 ...
 */
export type PrimeRadiantEvent = 'pr:initialized' | 'pr:shutdown' | 'pr:coherence-check' | 'pr:coherence-violation' | 'pr:spectral-analysis' | 'pr:causal-inference' | 'pr:consensus-verified' | 'pr:consensus-failed' | 'pr:topology-computed' | 'pr:cache-hit' | 'pr:cache-miss';
/**
