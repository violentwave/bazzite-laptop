...
export type TestPatternType = 'unit' | 'integration' | 'e2e' | 'property' | 'mutation' | 'fuzz' | 'api' | 'performance' | 'security' | 'accessibility' | 'contract' | 'bdd';
/**
...
export interface IQEMemoryBridge {
     ...
     */
    searchSimilarPatterns(query: string, k?: number, filters?: PatternFilters): Promise<TestPattern[]>;
    /**
     ...
     */
    searchTrajectories(query: string, k?: number, filters?: {
        taskType?: string;
        success?: boolean;
 ...
 */
export type PIIType = 'email' | 'phone' | 'ssn' | 'credit_card' | 'api_key' | 'password' | 'address' | 'name' | 'dob' | 'ip_address';
/**
...
export interface IQESecurityBridge {
     ...
     */
    validateInput<T>(input: unknown, schema: string): Promise<{
        valid: boolean;
        errors?: string[];
        value?: T;
    }>;
...
export interface IQEModelRoutingAdapter {
     ...
     */
    canUseAgentBooster(task: QETask): {
        available: boolean;
        intent?: string;
    };
}
