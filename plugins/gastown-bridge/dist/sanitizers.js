 ...
 * ```
 */
export function sanitizeBeadOutput(raw) {
    // Check size limit
    if (raw.length > MAX_OUTPUT_SIZE.single) {
        throw BeadsError.parseFailed(raw.slice(0, 100) + '...', new Error(`Output exceeds maximum size of ${MAX_OUTPUT_SIZE.single} bytes`));
    }
    // Parse JSON
    let parsed;
    try {
    ...
    if (!result.success) {
        throw BeadsError.parseFailed(raw, new Error(result.error.errors.map(e => e.message).join('; ')));
    }
 ...
 * ```
 */
export function sanitizeFormulaOutput(raw) {
    // Check size limit
    if (raw.length > MAX_OUTPUT_SIZE.single) {
        throw FormulaError.parseFailed('output', `Output exceeds maximum size of ${MAX_OUTPUT_SIZE.single} bytes`);
    }
    // Parse JSON
    let parsed;
    try {
        ...
        throw FormulaError.parseFailed('output', 'Invalid JSON', error instanceof Error ? error : undefined);
    }
    // Redact sensitive fields before validation
    if (typeof parsed === 'object' && parsed !== null) {
        redactSensitiveFields(parsed);
    ...
    if (!result.success) {
        throw FormulaError.parseFailed('output', result.error.errors.map(e => e.message).join('; '));
    }
    const data = result.data;
 ...
 */
export function sanitizeConvoyOutput(raw) {
    // Check size limit
    if (raw.length > MAX_OUTPUT_SIZE.single) {
        throw ConvoyError.createFailed(`Output exceeds maximum size of ${MAX_OUTPUT_SIZE.single} bytes`);
    }
    // Parse JSON
    let parsed;
    try {
        ...
        throw ConvoyError.createFailed('Invalid JSON', error instanceof Error ? error : undefined);
    }
    ...
    if (!result.success) {
        throw ConvoyError.createFailed(result.error.errors.map(e => e.message).join('; '));
    }
    const data = result.data;
 ...
 */
export function sanitizeBeadsListOutput(raw) {
    // Check size limit
    if (raw.length > MAX_OUTPUT_SIZE.list) {
        throw BeadsError.parseFailed(raw.slice(0, 100) + '...', new Error(`Output exceeds maximum size of ${MAX_OUTPUT_SIZE.list} bytes`));
    }
...
function sanitizeMetadata(metadata) {
    ...
    for (const [key, value] of Object.entries(metadata)) {
        ...
        if (typeof value === 'string') {
            ...
            result[sanitizedKey] = value.slice(0, 100).map(v => typeof v === 'string' ? sanitizeString(v, 256) : v);
        }
        else if (typeof value === 'object' && value !== null) {
            result[sanitizedKey] = sanitizeMetadata(value);
        }
    }
    return result;
}
 ...
 */
function sanitizeVarsFromRaw(vars) {
    const result = {};
