...
export const BeadIdSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'Bead ID contains invalid characters')
    .refine((val) => !PATH_TRAVERSAL_PATTERNS.some(p => p.test(val)), 'Bead ID contains path traversal sequence')
    .refine((val) => BEAD_ID_PATTERN.test(val.trim()), 'Bead ID must be in format gt-{hash} or numeric')
...
export const FormulaNameSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'Formula name contains invalid characters')
    .refine((val) => !PATH_TRAVERSAL_PATTERNS.some(p => p.test(val)), 'Formula name contains path traversal sequence')
    .refine((val) => FORMULA_NAME_PATTERN.test(val.trim()), 'Formula name must start with letter and contain only alphanumeric, dash, or underscore')
    .transform(val => val.trim());
/**
...
export const ConvoyIdSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'Convoy ID contains invalid characters')
    .refine((val) => UUID_PATTERN.test(val.trim()) || CONVOY_HASH_PATTERN.test(val.trim()), 'Convoy ID must be a valid UUID or conv-{hash} format')
...
export const GtArgumentSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'Argument contains shell metacharacters')
    .refine((val) => !PATH_TRAVERSAL_PATTERNS.some(p => p.test(val)), 'Argument contains path traversal sequence');
/**
...
export const SafeStringSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'String contains shell metacharacters');
/**
...
export const RigNameSchema = z.string()
    ...
    .refine((val) => !SHELL_METACHARACTERS.test(val), 'Rig name contains invalid characters')
    .refine((val) => /^[a-zA-Z][a-zA-Z0-9-]{0,31}$/.test(val.trim()), 'Rig name must start with letter and contain only alphanumeric or dash')
    .transform(val => val.trim().toLowerCase());
/**
 * Schema for priority
 */
...
export const LabelsSchema = z.array(z.string()
    ...
    .refine((val) => /^[a-zA-Z][a-zA-Z0-9_-]*$/.test(val), 'Label must start with letter and contain only alphanumeric, dash, or underscore'))
 ...
 * ```
 */
export function validateBeadId(id) {
 ...
 * ```
 */
export function validateFormulaName(name) {
 ...
 * ```
 */
export function validateConvoyId(id) {
 ...
 * ```
 */
export function validateGtArgs(args) {
    if (!Array.isArray(args)) {
        throw new ValidationError('Arguments must be an array', GasTownErrorCode.INVALID_ARGUMENTS, [{ field: 'args', constraint: 'array', actual: typeof args }]);
    }
    ...
    if (!result.success) {
        ...
        throw new ValidationError('Invalid CLI arguments', GasTownErrorCode.INVALID_ARGUMENTS, constraints);
    }
    return result.data;
}
// ============================================================================
...
export const CreateBeadOptionsSchema = z.object({
    title: z.string()
        ...
        .refine((val) => !SHELL_METACHARACTERS.test(val), 'Title contains invalid characters'),
    ...
    assignee: z.string()
        ...
        .refine((val) => /^[a-zA-Z][a-zA-Z0-9_-]*$/.test(val), 'Invalid assignee name format')
        .optional(),
});
/**
...
export const CreateConvoyOptionsSchema = z.object({
    name: z.string()
        ...
        .refine((val) => !SHELL_METACHARACTERS.test(val), 'Convoy name contains invalid characters'),
 ...
 */
export function validateCreateBeadOptions(options) {
    ...
    if (!result.success) {
        ...
        throw new ValidationError('Invalid bead creation options', GasTownErrorCode.VALIDATION_FAILED, constraints);
    }
    return result.data;
}
/**
 ...
 */
export function validateCreateConvoyOptions(options) {
    ...
    if (!result.success) {
        ...
        throw new ValidationError('Invalid convoy creation options', GasTownErrorCode.VALIDATION_FAILED, constraints);
    }
    return result.data;
}
/**
 ...
 */
export function validateSlingOptions(options) {
    ...
    if (!result.success) {
        ...
        throw new ValidationError('Invalid sling options', GasTownErrorCode.VALIDATION_FAILED, constraints);
    }
    return result.data;
}
// ============================================================================
