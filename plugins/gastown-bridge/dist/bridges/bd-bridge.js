...
import { z } from 'zod';
import { LRUCache, BatchDeduplicator, Lazy, } from '../cache.js';
const execFileAsync = promisify(execFile);
// ============================================================================
// Performance Caches
// ============================================================================
/** Result cache for memoizing bead queries */
const beadQueryCache = new LRUCache({
    maxEntries: 100,
    ttlMs: 30 * 1000, // 30 sec TTL (beads may change)
});
/** Single bead cache */
const singleBeadCache = new LRUCache({
    maxEntries: 500,
...
const execDedup = new BatchDeduplicator();
/** Parsed JSONL cache */
...
const SafeStringSchema = z.string()
    ...
    .refine((val) => !/[;&|`$(){}><\n\r\0]/.test(val), 'String contains shell metacharacters');
/**
...
export class BdBridgeError extends Error {
    ...
    args;
    cause;
    constructor(message, code, command, args, cause) {
        super(message);
        this.code = code;
...
export class BdBridge {
    ...
    static CACHEABLE_COMMANDS = new Set([
        'list',
        'get',
        'search',
        'stats',
    ...
    static STATIC_COMMANDS = new Set([
        'version',
        'help',
        'stats',
        'config',
    ...
    async initialize() {
        try {
            ...
            if (!result.success) {
                throw new BdBridgeError('Failed to verify bd installation', 'COMMAND_NOT_FOUND', 'bd', ['version']);
            }
            ...
            if (error instanceof BdBridgeError)
                throw error;
            throw new BdBridgeError('Failed to initialize Beads bridge', 'COMMAND_NOT_FOUND', 'bd', ['version'], error);
        }
    }
    /**
     ...
     */
    async execBd(args, skipCache = false) {
        ...
        if (subcommand && !ALLOWED_BD_COMMANDS.has(subcommand)) {
            throw new BdBridgeError(`Command not allowed: ${subcommand}`, 'INVALID_ARGUMENT', 'bd', validatedArgs);
        }
        ...
        return execDedup.dedupe(cacheKey, async () => {
            try {
                ...
                const { stdout, stderr } = await execFileAsync(this.config.bdPath, validatedArgs, {
                    cwd: this.config.cwd,
                    env: this.config.env,
                ...
                const err = error;
                if (err.killed) {
                    throw new BdBridgeError('Command execution timed out', 'TIMEOUT', 'bd', validatedArgs);
                }
                if (err.code === 'ENOENT') {
                    throw new BdBridgeError(`bd executable not found at: ${this.config.bdPath}`, 'COMMAND_NOT_FOUND', 'bd', validatedArgs);
                }
                return {
                    success: false,
    ...
    execBdStreaming(args) {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            childProcess.on('error', (error) => {
                reject(new BdBridgeError(error.message, 'EXECUTION_FAILED', 'bd', validatedArgs));
            });
        });
        return {
     ...
     */
    parseSingleBead(output) {
        if (!output || output.trim() === '') {
            throw new BdBridgeError('Empty output from bd command', 'INVALID_OUTPUT');
        }
        try {
            ...
            throw new BdBridgeError('Failed to parse bead output', 'PARSE_ERROR', undefined, undefined, error);
        }
    }
    /**
     * List beads with optional query parameters
     */
    async listBeads(query) {
        ...
        if (!result.success) {
            throw new BdBridgeError(result.error ?? 'Failed to list beads', 'EXECUTION_FAILED', 'bd', args);
        }
    ...
    async getBead(beadId) {
        ...
        if (!result.success) {
            if (result.error?.includes('not found')) {
                throw new BdBridgeError(`Bead not found: ${beadId}`, 'BEAD_NOT_FOUND', 'bd', args);
            }
            throw new BdBridgeError(result.error ?? 'Failed to get bead', 'EXECUTION_FAILED', 'bd', args);
        }
    ...
    async createBead(params) {
        ...
        const result = await this.execBd(args);
        if (!result.success) {
            throw new BdBridgeError(result.error ?? 'Failed to create bead', 'EXECUTION_FAILED', 'bd', args);
        }
     ...
     */
    async searchBeads(query, options) {
        this.ensureInitialized();
        const args = [
            'search',
        ...
        const result = await this.execBd(args);
        if (!result.success) {
            throw new BdBridgeError(result.error ?? 'Bead search failed', 'EXECUTION_FAILED', 'bd', args);
        }
        return this.parseBdOutput(result.data ?? '');
    }
    /**
    ...
    async exportBeads(query) {
        ...
        const result = await this.execBd(args);
        if (!result.success) {
            throw new BdBridgeError(result.error ?? 'Export failed', 'EXECUTION_FAILED', 'bd', args);
        }
        return result.data ?? '';
    }
    /**
    ...
    async getStats() {
        ...
        const result = await this.execBd(args);
        if (!result.success) {
            throw new BdBridgeError(result.error ?? 'Failed to get stats', 'EXECUTION_FAILED', 'bd', args);
        }
        try {
            return JSON.parse(result.data ?? '{}');
        }
        catch {
            throw new BdBridgeError('Failed to parse stats output', 'PARSE_ERROR', 'bd', args);
        }
    }
    /**
    ...
    validateAndSanitizeArgs(args) {
        return args.map((arg, index) => {
            try {
                ...
                throw new BdBridgeError(`Invalid argument at index ${index}: ${arg}`, 'VALIDATION_ERROR', 'bd', args, error);
            }
        });
    }
    /**
    ...
    ensureInitialized() {
        if (!this.initialized) {
            throw new BdBridgeError('Beads bridge not initialized. Call initialize() first.', 'EXECUTION_FAILED');
        }
    }
    /**
 ...
 */
export function createBdBridge(config, logger) {
...
//# sourceMappingURL=bd-bridge.js.map