...
export class GtBridge {
    ...
    async initialize() {
        try {
            ...
            if (!result.success) {
                throw new GtBridgeError('Failed to verify gt installation', 'COMMAND_NOT_FOUND', 'gt', ['version']);
            }
            ...
            if (error instanceof GtBridgeError)
                throw error;
            throw new GtBridgeError('Failed to initialize Gas Town bridge', 'COMMAND_NOT_FOUND', 'gt', ['version'], error);
        }
    }
    /**
     ...
     */
    async execGt(args, skipCache = false) {
        ...
        if (subcommand && !ALLOWED_GT_COMMANDS.has(subcommand)) {
            throw new GtBridgeError(`Command not allowed: ${subcommand}`, 'INVALID_ARGUMENT', 'gt', validatedArgs);
        }
        ...
        return execDedup.dedupe(cacheKey, async () => {
            try {
                ...
                const { stdout, stderr } = await execFileAsync(this.config.gtPath, validatedArgs, {
                    cwd: this.config.cwd,
                    env: this.config.env,
                ...
                const err = error;
                if (err.killed) {
                    throw new GtBridgeError('Command execution timed out', 'TIMEOUT', 'gt', validatedArgs);
                }
                if (err.code === 'ENOENT') {
                    throw new GtBridgeError(`gt executable not found at: ${this.config.gtPath}`, 'COMMAND_NOT_FOUND', 'gt', validatedArgs);
                }
                return {
                    success: false,
     ...
     */
    parseGtOutput(output) {
        if (!output || output.trim() === '') {
            throw new GtBridgeError('Empty output from gt command', 'INVALID_OUTPUT');
        }
        ...
        try {
            ...
            if (jsonMatch) {
                try {
                    ...
                    throw new GtBridgeError('Failed to parse gt output as JSON', 'INVALID_OUTPUT');
                }
            }
            throw new GtBridgeError('Output is not valid JSON', 'INVALID_OUTPUT');
        }
    }
    /**
    ...
    async estimateGas(params) {
        ...
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Gas estimation failed', 'EXECUTION_FAILED', 'gt', args);
        }
     ...
     */
    async getTxStatus(txHash, network) {
        this.ensureInitialized();
        ...
        const result = await this.execGt(args);
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Failed to get transaction status', 'EXECUTION_FAILED', 'gt', args);
        }
        return this.parseGtOutput(result.data);
    }
    /**
    ...
    async getNetworkStatus(network) {
        ...
        const result = await this.execGt(args);
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Failed to get network status', 'NETWORK_ERROR', 'gt', args);
        }
        return this.parseGtOutput(result.data);
    }
    /**
    ...
    async getGasPrice(network) {
        ...
        const result = await this.execGt(args);
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Failed to get gas price', 'EXECUTION_FAILED', 'gt', args);
        }
        return this.parseGtOutput(result.data);
    }
    /**
    ...
    async simulate(params) {
        ...
        const result = await this.execGt(args);
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Transaction simulation failed', 'EXECUTION_FAILED', 'gt', args);
        }
        return this.parseGtOutput(result.data);
    }
    /**
    ...
    async decode(data, abi) {
        ...
        const result = await this.execGt(args);
        if (!result.success || !result.data) {
            throw new GtBridgeError(result.error ?? 'Failed to decode transaction data', 'EXECUTION_FAILED', 'gt', args);
        }
        return this.parseGtOutput(result.data);
    }
    /**
    ...
    validateAndSanitizeArgs(args) {
        return args.map((arg, index) => {
            try {
                ...
                throw new GtBridgeError(`Invalid argument at index ${index}: ${arg}`, 'VALIDATION_ERROR', 'gt', args, error);
            }
        });
    }
    /**
    ...
    ensureInitialized() {
        if (!this.initialized) {
            throw new GtBridgeError('Gas Town bridge not initialized. Call initialize() first.', 'EXECUTION_FAILED');
        }
    }
    /**
 ...
 */
export function createGtBridge(config, logger) {
...
//# sourceMappingURL=gt-bridge.js.map