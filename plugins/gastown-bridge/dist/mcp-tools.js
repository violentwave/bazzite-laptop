 ...
 */
export const formulaExecuteTool = {
    ...
    handler: async (input, context) => {
        ...
        try {
            ...
            const bridge = context.bridges.gastown;
            const { beads_created } = await bridge.executeFormula(validated.formula, validated.vars, validated.target_agent, validated.dry_run);
            const result = {
                success: true,
 ...
 */
export const formulaCreateTool = {
 ...
 */
export const wasmParseFormulaTool = {
 ...
 */
export const wasmResolveDepsTool = {
 ...
 */
export const wasmCookBatchTool = {
    ...
    handler: async (input, context) => {
        ...
        try {
            ...
            const wasmStart = Date.now();
            const { cooked, errors } = await formulaWasm.cookBatch(validated.formulas, validated.vars, validated.continue_on_error);
            const wasmDuration = Date.now() - wasmStart;
            const result = {
 ...
 */
export const wasmMatchPatternTool = {
    ...
    handler: async (input, context) => {
        ...
        try {
            ...
            const wasmStart = Date.now();
            const matches = await depWasm.matchPatterns(validated.query, validated.candidates, validated.k, validated.threshold);
            const wasmDuration = Date.now() - wasmStart;
            const result = {
                success: true,
                matches,
 ...
 */
export const wasmOptimizeConvoyTool = {
    ...
    handler: async (input, context) => {
        ...
        try {
            ...
            const wasmStart = Date.now();
            const optimization = await depWasm.optimizeConvoy(convoy, validated.strategy, validated.resource_constraints);
            const wasmDuration = Date.now() - wasmStart;
            const result = {
                success: true,
                optimization,
 ...
 */
export const gasTownBridgeTools = [
    // Beads Integration (5 tools) - CLI Bridge
    beadsCreateTool,
    beadsReadyTool,
    beadsShowTool,
...
export const toolHandlers = new Map([
    // Beads tools
    ['gt_beads_create', beadsCreateTool.handler],
    ['gt_beads_ready', beadsReadyTool.handler],
    ['gt_beads_show', beadsShowTool.handler],
    ['gt_beads_dep', beadsDepTool.handler],
    ['gt_beads_sync', beadsSyncTool.handler],
    // Convoy tools
    ['gt_convoy_create', convoyCreateTool.handler],
    ['gt_convoy_status', convoyStatusTool.handler],
    ['gt_convoy_track', convoyTrackTool.handler],
    // Formula tools
    ['gt_formula_list', formulaListTool.handler],
    ['gt_formula_cook', formulaCookTool.handler],
    ['gt_formula_execute', formulaExecuteTool.handler],
    ['gt_formula_create', formulaCreateTool.handler],
    // Orchestration tools
    ['gt_sling', slingTool.handler],
    ['gt_agents', agentsTool.handler],
    ['gt_mail', mailTool.handler],
    // WASM tools
    ['gt_wasm_parse_formula', wasmParseFormulaTool.handler],
    ['gt_wasm_resolve_deps', wasmResolveDepsTool.handler],
    ['gt_wasm_cook_batch', wasmCookBatchTool.handler],
    ['gt_wasm_match_pattern', wasmMatchPatternTool.handler],
    ['gt_wasm_optimize_convoy', wasmOptimizeConvoyTool.handler],
]);
/**
