...
class PerfProfiler:
    ...
    def profile_llm_latency(
        self, endpoint: str = "http://127.0.0.1:8767/v1", n: int = 5
    ) -> LatencyStats:
    ...
    def profile_mcp_tools(
        self, endpoint: str = "http://127.0.0.1:8766", tools: list[str] | None = None, n: int = 3
    ) -> dict[str, LatencyStats]:
        """Call each MCP tool n times via JSON-RPC. Uses JSON-RPC tools/list."""
        tool_stats: dict[str, LatencyStats] = {}
        ...
        if tools is None:
            try:
                ...
                tools = [
                    t["name"]
                    for t in tool_list
                    if not t.get("annotations", {}).get("destructive", False)
                ]
            except Exception as e:
                self.errors.append(f"Failed to fetch MCP tools: {e}")
                return tool_stats

        for tool_name in tools[:20]:
            samples: list[float] = []
    ...
    def profile_file_io(self, paths: list[str] | None = None) -> list[FileIOProfile]:
        """Measure read latency for each path. Reports missing files with size_bytes=-1."""
        if paths is None:
            home = Path.home()
            paths = [
    ...
    def profile_lancedb(self, db_path: str) -> list[LanceDBProfile]:
        """Profile LanceDB tables with vector search."""
        if not HAS_LANCEDB:
    ...
    def collect_system_snapshot(self) -> SystemSnapshot | None:
        """Return system metrics using psutil or /proc fallback."""
        try:
            cpu_percent = 0.0
            memory_used_mb = 0.0
    ...
    def generate_report(self, results: ProfileReport, output_path: str) -> dict[str, Any]:
        ...
        if results.system_snapshot:
            ...
            print(
                f"  Memory: {s.memory_used_mb:.0f}MB used, {s.memory_available_mb:.0f}MB available"
            )
            print(f"  Load (1m): {s.load_avg_1m:.2f}")

        if results.llm_profile:
            llat = results.llm_profile.latency
            print(f"\nLLM Latency ({results.llm_profile.endpoint}):")
            print(
                f"  Min: {llat.min_ms:.1f}ms, Mean: {llat.mean_ms:.1f}ms, P95: {llat.p95_ms:.1f}ms"
            )
    ...
    def run(self, output_dir: str, skip: list[str] | None = None) -> str:
        ...
        report_file = (
            output_path / f"perf_profile_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json"
        )
