...
class IntelScraper:
    ...
    def scrape_github_releases(self, owner: str, repo: str, since_days: int = 7) -> list[dict]:
    ...
    def scrape_cisa_kev(self, since_days: int = 3) -> list[dict]:
        ...
        try:
            ...
            for vuln in data.get("vulnerabilities", []):
                ...
                try:
                    add_dt = datetime.strptime(date_added, "%Y-%m-%d").replace(tzinfo=UTC)
                    if add_dt < since:
                        continue
                except ValueError:
                    continue
    ...
    def scrape_nvd_cves(self, keywords: list[str] | None = None, days: int = 7) -> list[dict]:
    ...
    def run_all(self, output_dir: str) -> dict:
        ...
        try:
            cves = self.scrape_nvd_cves(
                keywords=["python", "linux", "fedora", "nvidia", "node", "npm"], days=7
            )
            if cves:
...
def main():
    ...
    parser.add_argument(
        "--source", choices=["github", "cisa", "nvd", "fedora"], help="Run only specific source"
    )
